...
#define ABK_META_MOUNT_RUNTIME_ID "abk-meta-mount-runtime"
#define ABK_META_MOUNT_RUNTIME_DIR ABK_META_MOUNT_MODULES_DIR "/" ABK_META_MOUNT_RUNTIME_ID
#define ABK_META_MOUNT_RUNTIME_NAME "ABK Meta Mount Runtime"
...
static int abk_meta_mount_ensure_compat_module(void)
{
	static const char script[] =
		"set -eu\n"
		"MOD='" ABK_META_MOUNT_DATA_DIR "'\n"
		"RTMOD='" ABK_META_MOUNT_RUNTIME_DIR "'\n"
		"WEB='" ABK_META_MOUNT_WEB_ROOT "'\n"
		"MARK='" ABK_META_MOUNT_MARKER "'\n"
		"mkdir -p \"$MOD\" \"$RTMOD\" \"$WEB\"\n"
		...
		"action=1\n"
		"ABK_META_PROP\n"
		"cat > \"$RTMOD/module.prop\" <<'ABK_META_RUNTIME_PROP'\n"
		"id=" ABK_META_MOUNT_RUNTIME_ID "\n"
		"name=" ABK_META_MOUNT_RUNTIME_NAME "\n"
		"version=" ABK_META_MOUNT_VERSION "\n"
		"versionCode=1\n"
		"author=" ABK_META_MOUNT_AUTHOR "\n"
		"description=Staging helper for " ABK_META_MOUNT_NAME "\n"
		"ABK_META_RUNTIME_PROP\n"
		": > \"$RTMOD/skip_mount\"\n"
		"cat > \"$RTMOD/stage.sh\" <<'ABK_META_STAGE'\n"
		"#!/system/bin/sh\n"
		"set -eu\n"
		"TARGET_MOD='" ABK_META_MOUNT_DATA_DIR "'\n"
		"RUNTIME_BASE='" ABK_META_MOUNT_RUNTIME_ROOT "'\n"
		"STAGE='" ABK_META_MOUNT_STAGE_MODULES_DIR "'\n"
		"STAGE_TMP='" ABK_META_MOUNT_RUNTIME_ROOT "/modules.tmp'\n"
		"OVERLAY='" ABK_META_MOUNT_OVERLAY_DIR "'\n"
		"[ -d '" ABK_META_MOUNT_MODULES_DIR "' ] || exit 0\n"
		"[ -f \"$TARGET_MOD/disable\" ] && exit 0\n"
		"[ -f \"$TARGET_MOD/remove\" ] && exit 0\n"
		"rm -rf \"$STAGE_TMP\"\n"
		"mkdir -p \"$STAGE_TMP\" \"$OVERLAY\"\n"
		"for CUR in " ABK_META_MOUNT_MODULES_DIR "/*; do\n"
		"  [ -d \"$CUR\" ] || continue\n"
		"  MODNAME=${CUR##*/}\n"
		"  [ \"$MODNAME\" = '" ABK_META_MOUNT_ID "' ] && continue\n"
		"  [ \"$MODNAME\" = '" ABK_META_MOUNT_RUNTIME_ID "' ] && continue\n"
		...
		"ABK_META_STAGE\n"
		"chmod 755 \"$RTMOD/stage.sh\"\n"
		"cat > \"$RTMOD/post-fs-data.sh\" <<'ABK_META_POST'\n"
		"#!/system/bin/sh\n"
		"MODDIR=${0%/*}\n"
		"sh \"$MODDIR/stage.sh\" >/dev/null 2>&1 || true\n"
		"ABK_META_POST\n"
		"chmod 755 \"$RTMOD/post-fs-data.sh\"\n"
		"cat > \"$RTMOD/service.sh\" <<'ABK_META_SERVICE'\n"
		"#!/system/bin/sh\n"
		"MODDIR=${0%/*}\n"
		"[ -d '" ABK_META_MOUNT_STAGE_MODULES_DIR "' ] && exit 0\n"
		"sleep 15\n"
		"sh \"$MODDIR/stage.sh\" >/dev/null 2>&1 || true\n"
		"ABK_META_SERVICE\n"
		"chmod 755 \"$RTMOD/service.sh\"\n"
		...
		"if [ -x /data/adb/modules/" ABK_META_MOUNT_RUNTIME_ID "/stage.sh ]; then sh /data/adb/modules/" ABK_META_MOUNT_RUNTIME_ID "/stage.sh >/dev/null 2>&1 || true; fi\n"
