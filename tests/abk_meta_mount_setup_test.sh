...
assert_contains 'abk-meta-mount-runtime' "$driver"
assert_contains 'post-fs-data.sh' "$driver"
assert_contains 'service.sh' "$driver"
assert_contains 'stage.sh' "$driver"
assert_contains 'cp -a \"$SRC\"/. \"$DST\"/' "$driver"
assert_contains 'stage_missing' "$driver"
